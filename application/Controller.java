...
public class Controller {
	...
	public static void MoveRight(Form form) {
		// get value of then add 25px <= 300-25=275
		if (form.a.getX() + MOVE <= XMAX - SIZE && form.b.getX() + MOVE <= XMAX - SIZE
				&& form.c.getX() + MOVE <= XMAX - SIZE && form.d.getX() + MOVE <= XMAX - SIZE) {
		// converting into int first and then assigning index into variable
			//eg: movea=Mesh[2][3]
			int movea = MESH[((int) form.a.getX() / SIZE) + 1][((int) form.a.getY() / SIZE)];
			int moveb = MESH[((int) form.b.getX() / SIZE) + 1][((int) form.b.getY() / SIZE)];
			int movec = MESH[((int) form.c.getX() / SIZE) + 1][((int) form.c.getY() / SIZE)];
			int moved = MESH[((int) form.d.getX() / SIZE) + 1][((int) form.d.getY() / SIZE)];
//			 if array is filled with 1 means there a box then no move on right
			if (movea == 0 && movea == moveb && moveb == movec && movec == moved) {
//				  setting position of  every rectangle
...
//			System.out.println(a.getX());
			b.setX(XMAX / 2 - SIZE);
//			System.out.println("SHah"+(XMAX/2-SIZE));
			b.setY(SIZE);
			c.setX(XMAX / 2);
			c.setY(SIZE);
			d.setX(XMAX / 2 + SIZE);
...
//		 For creating whole block calling constructor
